...
export default function AddFlashcardsScreen({ navigation }) {
  ...
  const scaleAnim = useRef(new Animated.Value(1)).current; // ✅ animation for button feedback
  ...
  const handleAddPress = () => {
    ...
    ]).start(() => navigation.navigate("FlashcardsAdd"));
  };
  ...
  return (
    <ImageBackground source={require("../assets/bgg.png")} style={styles.container}>
      ...
      {/* ➕ Add Flashcards Button (Animated) */}
      <Animated.View style={{ transform: [{ scale: scaleAnim }] }}>
        <TouchableOpacity style={styles.addButton} onPress={handleAddPress}>
          <Ionicons
            ...
            style={{ marginRight: 8 }}
          />
          <Text style={styles.addButtonText}>Add Flashcards</Text>
        </TouchableOpacity>
      </Animated.View>
